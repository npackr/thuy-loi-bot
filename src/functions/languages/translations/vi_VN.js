...
export const OR_YOU_CAN_ASK_EVERYTHING_BY = `Hoặc bạn cũng có thể tán gẫu với ${BOT_NAME} bằng bất cứ vấn đề nào. Ví dụ: Em ơi, em ăn cơm chưa?; Em ơi, làm sao để đậu đại học?; Em ơi, ...`;
export const CAN_NOT_REPLY_YOUR_QUESTION = `${BOT_NAME} hiện chưa thể trả lời câu hỏi này`;
export const PLEASE_LET_ME_KNOW_WHAT_YOU_WANT_TO_SAY = `Vui lòng cho ${BOT_NAME} biết bạn muốn nói gì. Ví dụ: Em ơi, anh muốn học thật giỏi,...`;
...
export const ADMISSION_INSTRUCTION = `Để xem thông tin tuyển sinh của Phân hiệu Đại học Thủy lợi bạn có thể sử dụng lệnh \` /admission \` của ${BOT_NAME} hoặc truy cập trang thông tin tuyển sinh: https://tlus.edu.vn/tuyen-sinh`;
export const REGISTER_INSTRUCTION = `Để đăng ký nhập học của Phân hiệu Đại học Thủy lợi bạn có thể sử dụng lệnh \` /register \` của ${BOT_NAME} hoặc truy cập trang đăng ký nhập học: https://nhaphoc.tlus.edu.vn`;
